...
(function ($, window, document, undefined) {
  ...
  function arrayCompare(arrayA, arrayB) {
    if (!arrayB) return false;

    if (arrayA.length != arrayB.length) return false;

    for (var i = 0; i < arrayA.length; i++) {
      if (arrayA[i] instanceof Array && arrayB[i] instanceof Array) {
        if (!arrayCompare(arrayA[i], arrayB[i])) return false;
      ...
      $.extend(throwable.prototype, {
        ...
        initElem: function (elem) {
          ...
          if (this.defaults.shape === "box")
            body = this.createBox(
              ...
              $elem.position().top + ($elem.outerHeight() >> 1),
              $elem.width() / 2,
              $elem.outerHeight() / 2,
              false,
              numInstance,
            ...
            body = this.createCircle(
              ...
              $elem.position().left + ($elem.width() >> 1),
              $elem.position().top + ($elem.outerHeight() >> 1),
              Math.max($elem.width() / 2, $elem.outerHeight() / 2),
              false,
              numInstance,
              Math.pow(2, 50) - 1
            );
        ...
        getProjectedWidth: function (elem) {
          ...
          return (
            ...
            elem.outerHeight() * Math.sin(rotationAngle)
          );
        },
        getProjectedHeight: function (elem) {
          ...
          return (
            ...
            elem.outerHeight() * Math.cos(rotationAngle)
          );
        },
